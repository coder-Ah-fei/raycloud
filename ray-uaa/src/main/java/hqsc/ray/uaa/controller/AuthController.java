package hqsc.ray.uaa.controller;

import com.xkcoding.justauth.AuthRequestFactory;
import hqsc.ray.uaa.config.SocialConfig;
import hqsc.ray.uaa.enums.LoginType;
import hqsc.ray.uaa.service.ValidateService;
...
import hqsc.ray.system.feign.ISysRolePermissionProvider;
...
public class AuthController {
    ...
    public Result<?> getUser(HttpServletRequest request) {
        ...
        // 存入redis,以用于ray-starter-auth的PreAuthAspect查询权限使用
        redisService.set(Oauth2Constant.RAY_PERMISSION_PREFIX + loginUser.getAccount()
    ...
    public Map<String, String> loginType() {
        ...
        return oauthList.stream().collect(Collectors.toMap(oauth -> oauth.toLowerCase() + "登录", oauth -> "http://localhost:9999/ray-uaa/auth/login/" + oauth.toLowerCase()));
    }
