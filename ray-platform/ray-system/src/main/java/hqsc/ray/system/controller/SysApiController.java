 ...
 */
package hqsc.ray.system.controller;
...
import hqsc.ray.system.service.ISysApiService;
...
public class SysApiController extends BaseController {
	...
	public Result<?> sync() {
		Set<Object> serviceIds = redisService.sGet(RayConstant.RAY_SERVICE_RESOURCE);
		for (Object service : serviceIds) {
			if (redisService.hHasKey(RayConstant.RAY_API_RESOURCE, service.toString())) {
				Map<String, Object> apiMap = (Map<String, Object>) redisService.hget(RayConstant.RAY_API_RESOURCE, service.toString());
			...
			redisService.hdel(RayConstant.RAY_API_RESOURCE, service.toString());
		}
		redisService.del(RayConstant.RAY_SERVICE_RESOURCE);
