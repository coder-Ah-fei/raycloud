...
public class UploadUtils {
	...
	// 最大文件大小
	private long maxSize = StringUtil.toInt(ResourceUtil.getConfigByName("upload.maxSize"))*1024*1024;
	// 定义允许上传的文件扩展名
		...
		BASE64Decoder decoder = new BASE64Decoder(); 
		 try   
	        {  
	            ...
	            if (b.length > maxSize) { // 检查文件大小
	    			// TODO
	            	infos[1] = "上传文件大小超过限制";
	    		} else if (!Arrays.<String> asList(extMap.get(dirName).split(",")).contains(fileExt)) {// 检查扩展名
	    			infos[1] = "上传文件扩展名是不允许的扩展名。\n只允许" + extMap.get(dirName) + "格式。";
	    		} else {
	    			// 创建文件夹
					savePath += dirName + "/";
					saveUrl += dirName + "/";
	...
	public String[] saveMultipartFile(MultipartFile file) {
		...
		String fileExt = originalFileName.substring(originalFileName.indexOf(".")+1,originalFileName.length());
		 try{  
