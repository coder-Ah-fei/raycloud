package hqsc.ray.core.web.listener;
...
public class RequestMappingScanListener implements ApplicationListener<ApplicationReadyEvent> {
	...
	public void onApplicationEvent(@NotNull ApplicationReadyEvent event) {
		...
		try {
			...
			res.put("list",list);
			redisService.hset(RayConstant.RAY_API_RESOURCE, microService, res, 18000L);
			redisService.sSetAndTime(RayConstant.RAY_SERVICE_RESOURCE, 18000L, microService);
			log.info("资源扫描结果:serviceId=[{}] size=[{}] redis缓存key=[{}]", microService,  list.size(), RayConstant.RAY_API_RESOURCE);
		} catch (Exception e) {
