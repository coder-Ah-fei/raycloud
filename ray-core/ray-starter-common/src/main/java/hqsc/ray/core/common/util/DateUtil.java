...
public class DateUtil {
	...
	/**
	 * 格式化时间，返回  今天 12:00 或者昨天 12:00 或者前天 12:00 或者05/05 12:00
	 *
	 * @param dateTime
	 * @return
	 */
	public static String simpleFormat(LocalDateTime dateTime) {
		Date now = new Date();
		String monthStr = "";
		DateTime today = cn.hutool.core.date.DateUtil.beginOfDay(now);
		DateTime yesterday = cn.hutool.core.date.DateUtil.offsetDay(today, -1);
		DateTime beforeYesterday = cn.hutool.core.date.DateUtil.offsetDay(today, -2);
		DateTime date = DateTime.of(cn.hutool.core.date.DateUtil.beginOfDay(convertLDTTOCalendar(dateTime)));
		if (date.equals(today)) {
			...
			monthStr = format(dateTime, "MM/dd");
		}
		String format = format(dateTime, "HH:mm");
		return monthStr + " " + format;
	}
	
	public static String simpleFormat(Date dateTime) {
		Date now = new Date();
		String monthStr = "";
		DateTime today = cn.hutool.core.date.DateUtil.beginOfDay(now);
		DateTime yesterday = cn.hutool.core.date.DateUtil.offsetDay(today, -1);
		DateTime beforeYesterday = cn.hutool.core.date.DateUtil.offsetDay(today, -2);
		DateTime date = cn.hutool.core.date.DateUtil.beginOfDay(dateTime);
		if (date.equals(today)) {
			monthStr = "今天";
		} else if (date.equals(yesterday)) {
			monthStr = "昨天";
			...
			monthStr = cn.hutool.core.date.DateUtil.format(dateTime, "MM/dd");
		}
		String format = cn.hutool.core.date.DateUtil.format(dateTime, "HH:mm");
		return monthStr + " " + format;
	}
	
	/**
	 * 如果是今年，返回simpleFormat()的结果，否则返回yyyy-MM-dd HH:mm:ss
	 *
	 * @param dateTime
	 * @return
	 */
	public static String simpleFormatWithYear(LocalDateTime dateTime) {
		Date now = new Date();
		DateTime thisYear = cn.hutool.core.date.DateUtil.beginOfYear(now);
		DateTime date = DateTime.of(cn.hutool.core.date.DateUtil.beginOfYear(convertLDTTOCalendar(dateTime)));
		if (date.equals(thisYear)) {
			return simpleFormat(dateTime);
		} else {
			return format(dateTime, "yyyy-MM-dd HH:mm");
		}
	}
	
	public static String simpleFormatWithYear(Date dateTime) {
		Date now = new Date();
		DateTime thisYear = cn.hutool.core.date.DateUtil.beginOfYear(now);
		DateTime date = cn.hutool.core.date.DateUtil.beginOfYear(dateTime);
		if (date.equals(thisYear)) {
			return simpleFormat(dateTime);
		} else {
			return cn.hutool.core.date.DateUtil.format(dateTime, "yyyy-MM-dd HH:mm");
		}
	}
