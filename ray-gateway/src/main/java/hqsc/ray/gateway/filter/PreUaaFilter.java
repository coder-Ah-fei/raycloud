...
public class PreUaaFilter implements GlobalFilter, Ordered {
	...
	private final RayApiProperties rayApiProperties;

	private final RedisService redisService;

	/**
	 * 路径前缀以/ray开头，如ray-system
	 */
	public static final String PATH_PREFIX = "/ray";
	...
	private boolean ignore(String path) {
		return rayApiProperties.getIgnoreUrl().stream()
				.map(url -> url.replace("/**", ""))
	...
	private Mono<Void> unauthorized(ServerHttpResponse resp, String msg) {
		return ResponseUtil.webFluxResponseWriter(resp, RayConstant.JSON_UTF8, HttpStatus.UNAUTHORIZED, msg); }

	@Override
	public int getOrder() {
		return RayConstant.RAY_UAA_FILTER_ORDER;
	}
